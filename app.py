...
st.write("왼쪽 사이드바에서 오늘 온 회원 명단을 편집하고 버튼을 누르면 테이블별로 조가 편성됩니다.")

st.markdown("---")

# 2. 초기 원본 명단 고정 (참석 열 완전 제거)
...
st.sidebar.write("오늘 안 온 회원은 행을 선택해 `Delete` 키로 지우거나, 맨 아래에 새 회원을 추가할 수 있습니다.")

# 컬럼 구성에서 참석 빼고 이름과 에버리지만 지정
...
edited_df = st.sidebar.data_editor(
    ...
    hide_index=True,
    key="liberte_editor_no_checkbox"
)
...
if st.button("🔥 지정된 테이블 수로 팀 짜기 시작 (클릭)", type="primary", use_container_width=True):
    ...
    if total_players < num_teams:
        st.error(f"🚨 현재 입력된 명단 인원({total_players}명)이 지정한 테이블 수({num_teams}개)보다 적습니다. 왼쪽 메뉴에서 테이블 수를 줄이거나 인원을 추가해 주세요.")
    else:
        try:
